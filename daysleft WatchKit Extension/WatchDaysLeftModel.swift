...
class WatchDaysLeftModel: DaysLeftModel {
    ...
    override func pushAllSettingsToWatch() {
        // Don't send settings to watch when on watch!
    }
        
    override func session(_ session: WCSession, didReceiveUserInfo userInfo: [String: Any]) {
        super.session(session, didReceiveUserInfo: userInfo)
        self.updateComplications()
    }

    override func session(_ session: WCSession, didReceiveUpdate receivedApplicationContext: [String: AnyObject]) {
